...
export default class ThreeOscSynth
{
  constructor()
  {
    ...
    let { synth, midi, osc1, osc2, osc3, filter, mixer, vca, envelope, lfo, reverb } = this;
    ...
    synth.addModule(vca);
    synth.addModule(reverb);

    synth.connect(osc1).to(mixer.ch1);
    synth.connect(osc2).to(mixer.ch2);
    synth.connect(osc3).to(mixer.ch3);
    synth.connect(mixer).to(vca).to(filter).to(reverb);

    reverb.setup().then(() => {
      synth.connect(reverb).output();
    });
