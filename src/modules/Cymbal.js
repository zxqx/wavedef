...
export default class Cymbal {
  ...
  _connect() {
    const { output, fm1, fm2, fm3, mixer, bpFilter, hpFilter, bpEnv, hpEnv, ampEnv, bpGain, hpGain } = this;
  ...
  setBpFrequency(frequency) {
    this.bpFilter.setFrequency(frequency);
  }

  setHpFrequency(frequency) {
    this.hpFilter.setFrequency(frequency);
  }

  setBpDepth(depth) {
    this.bpEnv.setDepth(depth);
  }

  setHpDepth(depth) {
    this.hpEnv.setDepth(depth);
  }

  setBpStart(start) {
    this.bpEnv.setStart(start);
  }

  setHpStart(start) {
    this.hpEnv.setStart(start);
  }
  ...
  getBpFrequency() {
    return this.bpFilter.getFrequency();
  }

  getHpFrequency() {
    return this.hpEnv.getStart();
  }
}
