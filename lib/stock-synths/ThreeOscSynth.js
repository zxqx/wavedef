...
export default class ThreeOscSynth
{
  constructor()
  {
    ...
    let { synth, midi, oscGroup, filter, mixer, vca, envelope, lfo, convolver, delay, eq3, computerKeyboard, frequencyAnalyzer, ringmod, chorus } = this;
    ...
    synth.connect(mixer).to(vca).to(filter).to(ringmod).to(delay).to(convolver).to(eq3).to(chorus).to(frequencyAnalyzer).output();
