...
export default class ThreeOscSynth
{
  constructor()
  {
    ...
    this.eq3 = new EQ3();
    this.computerKeyboard = new ComputerKeyboard(4);
    this.frequencyAnalyzer = new FrequencyAnalyzer();

    let { synth, midi, oscGroup, filter, mixer, vca, envelope, lfo, convolver, delay, eq3, computerKeyboard, frequencyAnalyzer } = this;
    ...
    synth.connect(mixer).to(vca).to(filter).to(delay).to(convolver).to(eq3).to(frequencyAnalyzer).output();
